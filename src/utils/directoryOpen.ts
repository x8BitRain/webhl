...
const folderRegex = new RegExp(name +
  '(platform|models|resource|save|overviews|media|scripts|logos|hw|cl_dlls|events|text|manual|classes|stats)', 'i')
const fileRegex = new RegExp(name + '(.wav|.wad|.bsp|.dem|.spr|.tga)', 'i')

const getFiles = async (dirHandle: any, recursive: boolean, path: string = dirHandle.name): Promise<any> => {
  const dirs = [];
  const files = [];
